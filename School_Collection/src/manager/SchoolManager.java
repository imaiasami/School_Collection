...
public class SchoolManager {
	...
	private List<Person> persons;				// 리스트로 바꾸기
	private String fileName = "data.abc";
	private File file;
	private InputStream is;
	private OutputStream os;
	private ObjectInputStream ois;			// Object 스트림이 자바랑 가장 가깝다. 가장 먼저 만들어야 한다.
	private ObjectOutputStream oos;
	private BufferedInputStream bis;
	private BufferedOutputStream bos;
	
	public void saveFile() {
		// 프로그램을 추가, 수정, 삭제 등이 발생할 때 persons의 내용을 파일로 저장하는 메소드
		try {
	...
	public void loadFile() {
		// 프로그램 시작 시, 파일에서 persons의 내용을 불러온다. 프로그램 시작 시 한 번만 실행한다.
		try {
...
//		}		// 이렇게 해도 된다.
		...
		/*
		 *  forEach문은 읽기 전용이다. 삭제에는 쓸 수 없다.
		 *  
		 *  for (타입 변수명 : 순회 가능한 객체, 
		 */
